...
| Push ingest | `internal/server/handlers_probe_report.go` | `POST /api/v1/probe/report`, public vhost. Auto-registers on a valid grant; `monitor.AcceptPushedResults` folds the batch and returns the count the agent uses as its watermark |
| Install auth | `internal/server/install_grants.go` | Both installers are kiosk-vhost-only; the binaries need a grant (a minted vantage token or the admin token). `installBaseURL` is what the UI must build commands from |
...
- **Outside-in direction**: two modes, and **push is the default**. In push the agent reports to `POST /api/v1/probe/report` (public vhost) and needs nothing inbound — no public address, no open port, no certificate of its own — so it works behind NAT. In pull hz dials the agent, which costs all of those plus a pinned certificate re-done on every address change. hz accepting the POST adds no exposure: it arrives on the same public edge these checks exist to verify. Either way, only public facts (served hostnames, public IP) cross the wire, and check rows are prefixed `ext:` and carry `Vantage`.
- **A push vantage registers itself.** First report bearing a valid install grant creates the config entry, named by the agent — so the operator pastes nothing back. The token is the identity; the name in the body is a label, and results are filed under the token's vantage so a misnamed agent cannot report as another.
- **Push has no poll loop, so a watchdog replaces reachability**: `sweepPushVantages` marks a vantage failed after three missed intervals. Without it a dead agent reads as every target frozen on its last good result. "Never reported" stays distinct from "stopped reporting".
- **The port map is what hz was told, not what is listening.** `DeriveHostPortMap` is derived from config; a host also runs things nobody registered. `hz ports next` now verifies its candidate against the host (`/api/v1/ports/observed`, `internal/portscan`) before offering it, and `ports list` shows listening-but-unregistered ports. Without that it suggested a port a neighbouring daemon already held, and the service that took the advice restart-looped on a failed bind.
- **`dormant` is a reserved slot, not a disabled check.** The name, certificate and proxy entry stay; nothing is expected behind them. Backend reachability goes quiet (`svc:` check, and the vantage's `https` target), DNS and TLS stay live — those are what the reservation consists of and what has to be intact when the slot comes back. `StatusDormant` is distinct from `StatusDisabled`: one is a fact about the service, the other is "stop telling me about this check". **Every edit client must round-trip `dormant`** — the edit path assigns it from the request, so omitting it un-parks the service. `ServiceSchema` in `ui/src/api/schemas.ts` must list it too, or zod strips it before the form ever sees it.
- **`external_dns.ips` is a pin, not a default.** Empty means "track this host's public IP" and is what you want; a set value publishes that address forever, including after the host renumbers. `config.PinnedIPWarnings` surfaces pins that are not this host's address (and pins that redundantly match it, which break on the next change). `ExternalDNSResp.ConfiguredIPs` is the pinned value; `IPs` is the *effective* one and falls back to the host IP — reading `IPs` alone cannot tell the two apart.
- **The agent never executes what hz sends it.** hz reports the build it holds (`PushResponse.AgentVersion`, which is `s.version` — the embedded binaries are cross-compiled in the same build); the agent logs that it is behind and does nothing. Updating is a separate root `hz-probe-update.timer` running `hz-probe update`, because the agent runs as `DynamicUser` and cannot write its own binary or restart its unit. Keep it that way: an agent that acted on hz's word would turn a compromised hz into code execution on a host outside the network.
- **A registered vantage's own token authorises the binary download.** Install grants last an hour, which is right for a human pasting a command and useless for a nightly timer. Pull-mode installs get no timer — hz holds no token for them.
- **Vantage targets are per service, not per host IP.** `publicTargets` expects what the record is *supposed* to say (`GetPublicIPsForService`, which honours a pin), so a deliberately-pinned record is not reported as broken. A service with no proxy but with `external_dns` gets a DNS-only target — the WireGuard endpoint record matters and has nothing serving HTTP behind it.
...
- **Install routes are kiosk-only, and the binaries are not anonymous.** The installer is fetched by a host outside the network, so it lives on the public-facing (`kiosk_url`) vhost, not the admin hostname — on the admin name it 404s. The scripts stay anonymous (a few KB, no secret); the 7MB binaries require an install grant: a token hz minted for a command, redeemable for an hour, or the shared admin token (unless it is disabled). With no `kiosk_url` set there is no second vhost, so any host is allowed — the weaker posture an operator opted into by having no portal. **Never build an install command from `window.location.origin`** — it is the admin origin and will 404; use the `installBase` the mint endpoint returns.
