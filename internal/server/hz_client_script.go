...
COMMANDS
  ...
  Static site (static-folder services):
    ...
    Note: each push replaces the WHOLE site (fresh release, atomic swap).
          Files removed locally are gone after the next push — there is no
          incremental/merge mode, so no --purge flag is needed.
...
EXAMPLES
  ...
  hz-client site push ./public              # replace live site with ./public
  hz-client site push ./public --validate   # dry run: check the tarball, no swap
  hz-client site releases
...
# The wire contract this script was written against. It travels on every request
# so a server can refuse a script that is too old to understand its answers,
# instead of the mismatch surfacing as a field that silently reads empty.
#
# That is not hypothetical: the bans verb printed "created=- expires=never" for
# every ban for as long as this file has existed, because the server sends
# createdAt and this script read created_at. Nothing caught it. This header is
# what makes the next one of those a refusal naming both version numbers.
#
# Bump it in lockstep with hzapi.Version when this script is updated for a
# breaking change.
HZ_API_VERSION=1
AUTH_ARGS+=(-H "X-HZ-API-Version: $HZ_API_VERSION")
...
print()
for b in bans:
    created = datetime.fromtimestamp(b['createdAt']).strftime('%Y-%m-%d %H:%M:%S') if b.get('createdAt') else '-'
    expires = datetime.fromtimestamp(b['expiresAt']).strftime('%Y-%m-%d %H:%M:%S') if b.get('expiresAt') else 'never'
